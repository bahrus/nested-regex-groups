...
// ============================================================================
// PARSING STRATEGY: Single Regex vs Array of Regexes
// ============================================================================
//
// Q: Can nested-regex-groups parse all README scenarios with a single regex?
// A: NO - and you SHOULDN'T try! Here's why:
//
// SCENARIOS FROM README (simplified):
//
// 1. "on when #lhs equals #rhs"
// 2. "off when #lhs eq #rhs"
// 3. "on when #lhs?.weight gt #rhs?.weight"
// 4. "on when #lhs::change equals #rhs::input"
// 5. "on if nearlyEq, based on #lhs::change and #rhs::molting"
// 6. "on when isHappy"  (single boolean, no comparison)
// 7. "off when isHappy eq isWealthy"  (host properties, no #)
// 8. "on when #lhs equals `37`-as-number"  (constant comparison)
// 9. "on only when #isHappy"  (AND condition)
//
// COMPLEXITY ANALYSIS:
//
// A single regex to handle ALL of these would be:
// ❌ 500+ characters long
// ❌ Completely unreadable
// ❌ Impossible to debug
// ❌ Fragile - one change breaks everything
// ❌ Poor error messages - which part failed?
// ❌ Difficult to extend with new syntax
//
// Example of what a "unified" regex would look like:
// /^(?<trigger>on|off)\s+(?:(?:if|using)\s+(?<handler>\w+),?\s+(?:based\s+on|from)\s+)?(?:only\s+)?when\s+(?:(?<lhs.id>#(?:\{\{(?<lhs.idName>[^}]+)\}\}|(?<lhs.idName2>\w+)))(?:::(?<lhs.event>\w+))?(?:\?\.(?<lhs.prop>\w+))?(?:\s+as-(?<lhs.asType>\w+))?|(?<lhs.hostProp>\w+))\s*(?:(?<op>equals|eq|lt|gt|gte|lte|ne|not\s+equals)\s+(?:(?<rhs.id>#(?:\{\{(?<rhs.idName>[^}]+)\}\}|(?<rhs.idName2>\w+)))(?:::(?<rhs.event>\w+))?(?:\?\.(?<rhs.prop>\w+))?(?:\s+as-(?<rhs.asType>\w+))?|(?<rhs.hostProp>\w+)|`(?<rhs.constant>[^`]+)`(?:\s+as-(?<rhs.constType>\w+))?))?\.?$/
//
// ^ This is UNMAINTAINABLE! 🔥
//
// ============================================================================
// RECOMMENDED APPROACH: Array of Regexes (Priority Order)
// ============================================================================
//
// Instead, use an ARRAY of patterns, tried in order (first match wins):
...
const beSwitchedPatterns: ParsePattern[] = [
    // 1. Named handler with multiple dependencies (most specific first!)
    {
        name: 'namedHandlerMulti',
        regex: /^(?<trigger>on|off)\s+(?:if|using)\s+(?<handler>\w+),?\s+(?:based\s+on|from)\s+(?<deps>.+)$/,
        description: 'on if nearlyEq, based on #lhs::change and #rhs::molting'
    },
    
    // 2. Two-value comparison with full features (id, event, prop, type)
    {
        name: 'fullComparison',
        regex: /^(?<trigger>on|off)\s+when\s+(?<lhs.ref>#\{\{(?<lhs.id>\w+)\}\}|#(?<lhs.id2>\w+)|(?<lhs.hostProp>\w+))(?:::(?<lhs.event>\w+))?(?:\?\.(?<lhs.prop>\w+))?(?:\s+as-(?<lhs.asType>\w+))?\s+(?<op>equals|eq|lt|gt|gte|lte|ne)\s+(?<rhs.ref>#\{\{(?<rhs.id>\w+)\}\}|#(?<rhs.id2>\w+)|(?<rhs.hostProp>\w+))(?:::(?<rhs.event>\w+))?(?:\?\.(?<rhs.prop>\w+))?(?:\s+as-(?<rhs.asType>\w+))?\.?$/,
        description: 'on when #lhs::change?.weight equals #rhs::input?.weight'
    },
    
    // 3. Comparison with constant
    {
        name: 'constantComparison',
        regex: /^(?<trigger>on|off)\s+when\s+(?<lhs.ref>#\{\{(?<lhs.id>\w+)\}\}|#(?<lhs.id2>\w+)|(?<lhs.hostProp>\w+))(?:::(?<lhs.event>\w+))?(?:\?\.(?<lhs.prop>\w+))?\s+(?<op>equals|eq|lt|gt|gte|lte|ne)\s+`(?<rhs.constant>[^`]+)`(?:\s+as-(?<rhs.asType>\w+))?\.?$/,
        description: 'on when #lhs equals `37` as-number'
    },
    
    // 4. Simple comparison (no events, no props)
    {
        name: 'simpleComparison',
        regex: /^(?<trigger>on|off)\s+when\s+(?<lhs.ref>#(?<lhs.id>\w+)|(?<lhs.hostProp>\w+))\s+(?<op>equals|eq|lt|gt|gte|lte|ne)\s+(?<rhs.ref>#(?<rhs.id>\w+)|(?<rhs.hostProp>\w+))\.?$/,
        description: 'on when #lhs equals #rhs'
    },
    
    // 5. Single boolean condition with "only" (AND)
    {
        name: 'booleanAnd',
        regex: /^(?<trigger>on|off)\s+only\s+when\s+(?<lhs.ref>#\{\{(?<lhs.id>\w+)\}\}|#(?<lhs.id2>\w+)|(?<lhs.hostProp>\w+))\.?$/,
        description: 'on only when #isHappy'
    },
    
    // 6. Single boolean condition (simplest, must be last!)
    {
        name: 'booleanSimple',
        regex: /^(?<trigger>on|off)\s+when\s+(?<lhs.ref>#\{\{(?<lhs.id>\w+)\}\}|#(?<lhs.id2>\w+)|(?<lhs.hostProp>\w+))\.?$/,
        description: 'on when #isHappy or on when isHappy'
    }
];

// Parser that tries patterns in order
function parseBeSwitched(input: string): { pattern: string; result: any } | null {
    for (const pattern of beSwitchedPatterns) {
        const match = input.match(pattern.regex);
        if (match?.groups) {
            // Use nestedRegex to convert flat groups to nested
            const nested = flatToNested(match.groups);
            return {
...
// Helper to convert flat groups with dots to nested objects
function flatToNested(groups: Record<string, string>): any {
    const result: any = {};
...
// ============================================================================
// WHY ARRAY OF REGEXES IS BETTER
// ============================================================================
//
// ✅ READABILITY
//    - Each pattern is ~100 chars instead of 500+
//    - Can see what each pattern matches at a glance
//    - Easy to understand the grammar
//
// ✅ MAINTAINABILITY
//    - Add new syntax = add new pattern to array
//    - Modify syntax = change one pattern
//    - Remove syntax = remove one pattern
//    - No risk of breaking other patterns
//
// ✅ DEBUGGABILITY
//    - Know WHICH pattern matched
//    - Better error messages: "No pattern matched" vs "Regex failed"
//    - Can log which patterns were tried
//    - Easy to test each pattern independently
//
// ✅ EXTENSIBILITY
//    - Users can add custom patterns
//    - Plugins can register new patterns
//    - Priority order is explicit and configurable
//
// ✅ PERFORMANCE
//    - Most common patterns first = faster average case
//    - Can short-circuit on first match
//    - Can cache compiled regexes
//    - Can parallelize pattern matching (advanced)
//
// ✅ ERROR MESSAGES
//    - "Pattern 'simpleComparison' expected format: on when #lhs equals #rhs"
//    - vs "Regex failed at position 23"
//
// ============================================================================
// REAL-WORLD ANALOGY
// ============================================================================
//
// Think of it like HTTP routing:
//
// ❌ BAD: One giant regex to match all routes
//    /^\/(?:users\/(?<userId>\d+)|posts\/(?<postId>\d+)\/comments\/(?<commentId>\d+)|...)$/
//
// ✅ GOOD: Array of route patterns
//    [
//      { path: '/users/:userId', handler: getUser },
//      { path: '/posts/:postId/comments/:commentId', handler: getComment },
//      ...
//    ]
//
// Express.js, React Router, Vue Router all use arrays of patterns!
//
// ============================================================================
// IMPLEMENTATION RECOMMENDATION
// ============================================================================
//
// For nested-regex-groups package:
//
// 1. Core function: nestedRegex(pattern) - converts one regex
// 2. Helper function: tryPatterns(input, patterns[]) - tries array
// 3. Builder: createParser(patterns[]) - returns optimized parser
//
// Example API:
//
// import { nestedRegex, tryPatterns, createParser } from 'nested-regex-groups';
//
// // Single pattern
// const parser = nestedRegex(/^(?<user.name>\w+)@(?<user.domain>\w+)$/);
// parser('john@example.com'); // { user: { name: 'john', domain: 'example.com' } }
//
// // Multiple patterns (recommended for complex grammars)
// const beSwitchedParser = createParser([
//   { name: 'comparison', pattern: /^on when (?<lhs.id>#\w+) eq (?<rhs.id>#\w+)$/ },
//   { name: 'boolean', pattern: /^on when (?<lhs.id>#\w+)$/ }
// ]);
//
// beSwitchedParser('on when #lhs eq #rhs');
// // { matched: 'comparison', result: { lhs: { id: 'lhs' }, rhs: { id: 'rhs' } } }
//
// ============================================================================
// FINAL ANSWER
// ============================================================================
//
// Q: Single regex or array of regexes?
// A: ARRAY OF REGEXES - absolutely, without question!
//
// For be-switched specifically, I recommend 6-8 patterns covering:
// 1. Named handlers with dependencies
// 2. Full comparisons (with events, props, types)
// 3. Constant comparisons
// 4. Simple comparisons
// 5. Boolean AND conditions
// 6. Boolean simple conditions
//
// This gives you:
// - Readable code
// - Easy maintenance
...
// - Happy developers
//
// The nested-regex-groups package should make BOTH approaches easy,
// but strongly encourage the array approach through documentation and examples.
//
// ============================================================================